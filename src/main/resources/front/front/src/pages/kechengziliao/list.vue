...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0px","margin":"10px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","justifyContent":"","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
	  ...
	  <el-button v-if="btnAuth('kechengziliao','新增')" :style='{"cursor":"pointer","border":"1px solid #ccc","padding":"0px 8px","margin":"0px 10px 0 0","color":"#666","borderRadius":"0px","background":"none","display":"inline-block","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/kechengziliaoAdd')"><i v-if="true" :style='{"color":"#666","margin":"0 2px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="sort_view" :style='{"border":"1px solid #ddd","padding":"10px 20px","margin":"10px 0 0","overflow":"hidden","textAlign":"right","background":"none","width":"100%","order":"4"}'>
		...
		<el-button :style='{"border":"0","padding":"2px 0px","margin":"0 5px","borderRadius":"8px"}' @click="sortClick('thumbsupnum')">
			...
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#333"}' class="icon iconfont icon-jiantou23" v-else-if="sortType=='thumbsupnum'&&sortOrder=='desc'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#333"}' class="icon iconfont icon-jiantou24" v-else-if="sortType=='thumbsupnum'&&sortOrder=='asc'"></span>
	...
	<div class="list" :style='{"width":"calc(100% - 0px)","padding":"20px 0 0 0","margin":"0","background":"none","flex":"1","order":"6"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0 0 0 5px","margin":"10px 0 0","overflow":"hidden","color":"#666","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"100%","fontSize":"14px","height":"auto"}'>
			<div :style='{"border":"0px solid #ddd","cursor":"pointer","padding":"0","margin":"0 30px 30px 0","overflow":"hidden","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(25% - 30px)","position":"relative","justifyContent":"center","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<img @click.stop="imgPreView(item.xiangguantupian)" :style='{"border":"0px solid #eee","padding":"10px","margin":"0 0 10px","objectFit":"contain","background":"#f4f4f4","display":"block","width":"100%","height":"245px"}' v-if="item.xiangguantupian && item.xiangguantupian.substr(0,4)=='http'" :src="item.xiangguantupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.xiangguantupian?item.xiangguantupian.split(',')[0]:''))" :style='{"border":"0px solid #eee","padding":"10px","margin":"0 0 10px","objectFit":"contain","background":"#f4f4f4","display":"block","width":"100%","height":"245px"}' v-else :src="baseUrl + (item.xiangguantupian?item.xiangguantupian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"0 0px","borderColor":"#ddd","margin":"0px","whiteSpace":"nowrap","color":"#5A5A5A","textAlign":"center","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"30px","fontSize":"16px","textOverflow":"ellipsis","borderStyle":"solid"}' class="name ">{{item.kechengmingcheng}}</div>
				<div :style='{"padding":"0 0px","borderColor":"#ddd","margin":"0px","whiteSpace":"nowrap","color":"#5A5A5A","textAlign":"center","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"30px","fontSize":"16px","textOverflow":"ellipsis","borderStyle":"solid"}' class="name ">{{item.ziliaoleixing}}</div>
				...
				<div :style='{"padding":"0 0px","margin":"0 5px 0 0","display":"inline-block","order":"5"}'>
				  ...
				  <span class="text" :style='{"color":"#999","lineHeight":"1.5","fontSize":"inherit"}'>{{item.jiaoshizhanghao}}</span>
				</div>
				<div :style='{"padding":"0 0px","margin":"0 5px 0 0","display":"inline-block","order":"6"}'>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/ziliaoleixing/ziliaoleixing').then(res => {
			if (res.data.code == 0) {
				this.ziliaoleixingOptions = res.data.data;
				this.selectOptionsList.push({name:'资料类型',list:this.ziliaoleixingOptions,tableName: 'ziliaoleixing',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
